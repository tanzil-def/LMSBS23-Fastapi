...
def create_borrow(db: Session, request: borrow_schema.BorrowCreate):
    ...
    borrow = borrow_model.Borrow(
        ...
        status=borrow_model.BorrowStatusEnum.REQUESTED,
        extension_count=0
    )
    db.add(borrow)
    db.commit()
...
def return_book(db: Session, user_id: int, book_id: int):
    ...
    borrow.status = borrow_model.BorrowStatusEnum.RETURNED
    db.commit()
    db.refresh(borrow)
    return borrow
...
def get_borrow_stats(db: Session):
    ...
    active = db.query(func.count(borrow_model.Borrow.id)).filter(
        borrow_model.Borrow.status == borrow_model.BorrowStatusEnum.ACTIVE
    ).scalar()
    returned = db.query(func.count(borrow_model.Borrow.id)).filter(
        borrow_model.Borrow.status == borrow_model.BorrowStatusEnum.RETURNED
    ).scalar()
