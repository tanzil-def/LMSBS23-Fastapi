...
def format_book_urls(book: Book):
    """Add full URLs for cover, pdf_file, audio_file"""
    if book.cover and not book.cover.startswith("http"):
        book.cover = f"{MEDIA_URL}/covers/{book.cover}"
    if book.pdf_file and not book.pdf_file.startswith("http"):
        book.pdf_file = f"{MEDIA_URL}/pdfs/{book.pdf_file}"
