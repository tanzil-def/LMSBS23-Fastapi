...
def get_by_user_and_status(db: Session, user_id: int, status: BookingStatus):
    return db.query(Booking).filter(Booking.user_id == user_id, Booking.status == status).all()


def get_by_book_and_status(db: Session, book_id: int, status: BookingStatus):
    return db.query(Booking).filter(Booking.book_id == book_id, Booking.status == status).all()
...
def find_pending_booking_by_user_and_book(db: Session, user_id: int, book_id: int):
    return db.query(Booking).filter(
